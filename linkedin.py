...
def default_evade():
    ...
    """
    sleep(random.randint(2, 5))  # sleep a random duration to try and evade suspension
...
class Linkedin(object):
    ...
    def search_people(
        ...
        **kwargs,
        ...
        :param include_private_profiles: Include private profiles in search results. If False, only public profiles are
    ...
    def track(self, event_body, event_info):
        payload = {"eventBody": event_body, "eventInfo": event_info}
        res = self._post(
            "/li/track",
            base_request=True,
            headers={
    ...
    def _get_list_feed_posts_and_list_feed_urns(
        ...
        while True:
            ...
            if (
                    (-1 < limit <= len(l_urns))  # if our results exceed set limit
